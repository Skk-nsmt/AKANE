    ...
    if (!errors.isEmpty()) {
        ...
        mongoClient.connect(process.env.MONGODB_URI, function (err, client) {
            if (err) {
                ...
                const db = client.db(process.env.DATABASE_NAME)
                db.collection(process.env.DATABASE_COLLECTION).insertOne(newActivity, function (err, result) {
                    if (err) {
                        req.session.ifUnexpectedError = true
                        res.redirect("/add")
                    }
    ...
    if (!errors.isEmpty()) {
        ...
        mongoClient.connect(process.env.MONGODB_URI, function (err, client) {
            if (err) {
                ...
                const db = client.db(process.env.DATABASE_NAME)
                db.collection(process.env.DATABASE_COLLECTION).replaceOne({_id: ObjectId(req.query.id)}, newActivity, function (err, result) {
                    if (err) {
                        req.session.ifUnexpectedError = true
                        res.redirect("/edit?id" + req.query.id)
                    }
